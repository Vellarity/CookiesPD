  ...
  chrome.storage.sync.get(
    ...
    function (items) {
      ...
      if (search) {
        ...
        $(".search-cookie").css("display", "flex");
...
function set_cookie_handlers() {
  ...
  $(".edit-cookie").click(function () {
    ...
    $("#cookie-edit-value").val(
      decodeURIComponent(page_cookies[cookie_id].value)
    );
    $("#cookie-edit-expires").val(
      formatDate(new Date(page_cookies[cookie_id].expirationDate * 1000.0))
    );
    $("#cookie-edit-more-info").val(
      decodeURIComponent(JSON.stringify(page_cookies[cookie_id]))
    );
  });
...
function showCookieCount(cookie) {
  function showCount(count) {
    $("#cookie-count").html("<b>" + count + "</b> cookies found ");
...
function decCookieCount(times) {
  ...
  $("#cookie-count")
    .html("Found <b>" + new_val + "</b> cookies")
    .hide()
    .fadeIn();
...
function addRow(id, key, val) {
  function appendCookie(id, key, val) {
    $("#cookie-content").append(
      '<tr data-cookie-id="' +
        id +
        '" class="info_tr"><td>' +
        decodeURIComponent(key) +
        '</td> <td><button class="cookie-delete icon-button red" title="delete cookie"><svg> <use x="0" y="0" xlink:href="img/trash.svg#ico"></use></svg></button><button class="edit-cookie icon-button accent" title="edit-cookie"><svg> <use x="0" y="0" xlink:href="img/settings.svg#ico"></use></svg></button></td></tr>'
    );
  }
  if (!search) {
...
function notifySaved() {
  $("#notification-bar")
    .append('<span class="text-success">changes saved</span>')
    .hide()
    .fadeIn();
  setTimeout(function () {
    $("#notification-bar")
...
function notifyDeleted() {
  $("#notification-bar")
    .append('<span class="text-danger">cookie deleted</span>')
    .hide()
    .fadeIn();
  setTimeout(function () {
    $("#notification-bar")
